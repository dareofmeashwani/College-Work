...
<div class="display_noti">
    ...
    <div class="status">
        select to cancel appoitment<br>
        <form name="delappointment" method="post" action="delappointment.php">
        <?php
            ...
            {
                $sqlquery='DELETE FROM appointment WHERE id="'.$_POST['delapp'].'";';
                if ($conn->query($sqlquery) === FALSE) {echo "Error: " . $conn->error;die('unable to select database data');}
                else $msg="<br><br>Appoinment Successfully DELETE";
            }
            ...
            if($type=="patient") echo '<input type="radio" name="delapp" value="'.$row['id'].'"> Appointment Id: '.$row['id'].'&nbsp;&nbsp;&nbsp; Date: '.$row['date'].'&nbsp;&nbsp;&nbsp;   Timing '.$row['time'].'&nbsp;&nbsp;&nbsp;    Doctor :'.$row['name'].'</br>';
            if($type=="doctor") echo '<input type="radio" name="delapp" value="'.$row['id'].'"> Appointment Id: '.$row['id'].'&nbsp;&nbsp;&nbsp; Date: '.$row['date'].'&nbsp;&nbsp;&nbsp;   Timing '.$row['time'].'&nbsp;&nbsp;&nbsp;    Patient :'.$row['name'].'</br>';
            }
            }
            else
            {
            ...
            echo '<br><input name="appointmentdelete" type="submit" value="Delete Appointment" style="width: 18%">';
            echo '<br><br><br><br><br>';
            echo $msg;
        ?>
        </form>
