...
<div class="display_noti">
    ...
    <div class="status">
        <form name="appointmentform"action="scheduleoperation.php" method="post">
            <?php
            ...
            {
            $sqlquery='insert into opt_schedule values("'.gettimestamp().'","'.$_POST['hdate'].'","'.$_POST['answer'].'","'.$_POST['hdoctors'].'","'.$email.'");';
            if ($conn->query($sqlquery) === FALSE) {echo "Error: " . $conn->error;die('unable to select database data');}
            else $msg="<br><br>Operation Schdeuled Successfully";
            }
        ...
        <form name="addappointmentfinalizeform" action="scheduleoperation.php" method="post">
            <?php
        if(isset($_POST["appointmentsubmit"])&&$_POST['date']!="")
        {
            ...
            echo '<input name="appointmentfinalize" type="submit" value="Schedule Operation" style="width: 18%">';
            }
            ?>
        </form>
        <?php
