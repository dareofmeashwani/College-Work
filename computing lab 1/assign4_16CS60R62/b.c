...
#include<string.h>
#include <pwd.h>
character main (int argc, char **argv)
{
char currentdir[200];
char *line;
pid_t  pid;
char command[1000],argument[1000];
character i,c_end,status;
const char *homedir;
if ((homedir = getenv("HOME")) == NULL) {
	...
	do
	{
		...
		//prcharacterf("line:%s:command:%s:argument:%s:c_end:%d:length:%d:",line,command,argument,c_end,linelen);
		free(line);
		...
		else { /*strlwr(command);*/ if(strcmp(command,"exit")==0||line[0]=='\0') {} else prcharacterf("%s/> Command Not Found",currentdir);}		
		while(wait(&status)>0);
		...
		if(fp!=NULL)
		{
			...
			prcharacterf("current:%s",currentdir);
		}
		else {}
	}
